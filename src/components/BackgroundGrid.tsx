...
export const BackgroundGrid: React.FC<{ className?: string }> = ({ className }) => {
  return (
    <div className={`pointer-events-none absolute inset-0 -z-10 ${className || ''}`} aria-hidden="true">
      ...
      />
      <div className="absolute top-32 left-1/2 -translate-x-1/2 w-[60rem] h-[60rem] rounded-full bg-gradient-to-b from-indigo-100/60 via-white to-transparent blur-3xl opacity-60" />
    </div>
  );
};
